...
```bash
git clone https://github.com/your-org/dotnet-mcp-server
...
| Tool | Description |
|------|-------------|
| `analyze_c_sharp_file` | Returns structured metadata for one or more `.cs` files (batch mode supported) |
| `fetch_method_implementation` | Returns complete method body with exact line numbers (batch mode supported) |
| `read_file_content` | Reads raw file content; blocked for sensitive files |
| `analyze_method_call_graph` | Finds all callers and outgoing calls for a given method |
...
| `search_code_globally` | Keyword/name search across one or all projects |
...
| `edit_lines` | Apply targeted line-range patches atomically |
...
4. get_method_overloads(...)          — only if overloads are needed
```
...
```
LocalMcpServer/
├── MCPServers/              # MCP tool definitions (thin controllers over services)
...
- **No namespace update on file move.** `move_file` and `move_folder` move files and evict cache but do not rewrite `namespace` declarations or `using` directives in moved files. Update namespaces manually after a move.
- **No cache purge tool.** There is currently no MCP tool to force a full re-index of a project. If the cache gets stale after a large refactor, restart the server to trigger the startup indexer, or wait for the scheduled refresh.
